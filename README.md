...
Relay 是一个生产级的大规模时序数据可视化系统，基于 **Vue 3** (控制平面) 和 **Rerun** (渲染平面) 构建。它专为解决浏览器端渲染海量 3D/传感器数据的内存瓶颈而设计，采用**流式加载与主动内存管理 (Streaming & Active GC)** 架构。通过 Iframe 桥接与双向通信协议，Relay 将 Web 前端的业务逻辑与 WASM 的高性能渲染完美融合，支持**动态内存释放**、**交互式数据源切换**以及**高精度时间轴同步**，实现了在有限的浏览器资源下对无限时长时序数据的流畅回放。
