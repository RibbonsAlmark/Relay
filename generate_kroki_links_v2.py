...
def generate_kroki_url(mermaid_code):
    # Kroki requires RAW deflate (no zlib headers), then URL-safe Base64
    compressor = zlib.compressobj(9, zlib.DEFLATED, -15)
    compressed_data = compressor.compress(mermaid_code.encode('utf-8')) + compressor.flush()
    payload = base64.urlsafe_b64encode(compressed_data).decode('utf-8').rstrip("=")
